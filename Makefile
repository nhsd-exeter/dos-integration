...
integration-test: # End to end test DI project - mandatory: PROFILE, TAG=[complete|dev]; optional: ENVIRONMENT, PARALLEL_TEST_COUNT
	...
	echo RUN_ID=$$RUN_ID
	make -s docker-run-tester \
	CMD="sh -c 'python --version && pip show pytest-bdd pytest-xdist && pytest steps -k $(TAG) -vvvv --gherkin-terminal-reporter -p no:sugar -n $(PARALLEL_TEST_COUNT) --cucumberjson=./testresults.json --reruns 2 --reruns-delay 10'" \
