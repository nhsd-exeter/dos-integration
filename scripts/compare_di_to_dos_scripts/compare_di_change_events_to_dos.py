...
print("Loading files...")
di_cleaned_json = dynamodb_json.loads(open(di_json_path,).read())
...
df_di_latest_events = df_di_latest_events.join(pd.Series(opening_times_days, index=event_index, name="OpeningTimeWeekdays"))
df_di_latest_events = df_di_latest_events.join(pd.Series(opening_times_specified, index=event_index, name="OpeningTimeSpecified"))
df_di_latest_events = df_di_latest_events.join(pd.Series(nhs_uk_type, index=event_index, name="NhsUkType"))
df_di_latest_specified_openings = df_di_latest_events.drop(columns=["Event","SequenceNumber", "EventReceived", "TTL", "Id", "OpeningTimeWeekdays"])
