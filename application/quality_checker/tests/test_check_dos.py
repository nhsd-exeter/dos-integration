...
@patch(f"{FILE_PATH}.search_for_incorrectly_profiled_z_code")
def test_check_incorrect_zcode_profiling(
    mock_search_for_incorrectly_profiled_z_code: MagicMock,
    ...
    mock_search_for_incorrectly_profiled_z_code.return_value = matched_services
    # Act
    check_incorrect_zcode_profiling(connection, BLOOD_PRESSURE)
    # Assert
    mock_search_for_incorrectly_profiled_z_code.assert_called_once_with(connection, BLOOD_PRESSURE)
...
@patch(f"{FILE_PATH}.log_to_quality_check_report")
@patch(f"{FILE_PATH}.search_for_incorrectly_profiled_z_code")
def test_check_incorrect_zcode_profiling_no_matches(
    mock_search_for_incorrectly_profiled_z_code: MagicMock,
    mock_log_to_quality_check_report: MagicMock,
) -> None:
    # Arrange
