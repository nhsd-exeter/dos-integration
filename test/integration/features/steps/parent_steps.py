...
from features.utilities.utils import get_response, get_lambda_info, debug_purge_queue, process_payload, get_stored_events_from_db, get_payload
...
def the_change_event_is_processed(context):
    ...
    assert context.response.status_code == 200, f"Status code not as expected.. Status code: {context.response.status_code} Error: {message}"
...
def stored_dynamo_db_events_are_pulled(context, seqid: str, valid: str):
    ...
    assert expected_payload["SequenceNumber"] == db_event_record["SequenceNumber"], f'ERROR!!.. Change event record mismatch!!'
