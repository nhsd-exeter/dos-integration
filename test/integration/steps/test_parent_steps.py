...
def the_changed_contact_is_accepted_by_dos(context, contact):
    ...
    if contact == "phone_no":
        ...
        raise ValueError(f"Error!.. Input parameter '{contact}' not compatible")
    assert check_received_data_in_dos(context["correlation_id"], cms, changed_data) is True, "ERROR"
...
def the_changed_address_is_accepted_by_dos(context):
    ...
    assert check_received_data_in_dos(context["correlation_id"], "postaladdress", changed_address) is True, "ERROR"
