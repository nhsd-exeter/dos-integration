resource "aws_codebuild_webhook" "live_configuration-deployment_webhook" {
  ...
  filter_group {
    ...
    filter {
      type    = "HEAD_REF"
      pattern = "^refs/tags/.*-live-config-deploy"
    }
  }
}
resource "aws_codebuild_project" "di_deploy_live_configuration" {
  name           = "${var.project_id}-live-config-deploy-stage"
  description    = "Deploy configuration to the live environment"
  build_timeout  = "30"
  queued_timeout = "30"
  ...
  logs_config {
    cloudwatch_logs {
      group_name  = "/aws/codebuild/${var.project_id}-live-config-deploy-stage"
      stream_name = ""
    }
  }
...
resource "aws_codestarnotifications_notification_rule" "live_config_deploy_notification_rule" {
  detail_type    = "BASIC"
  ...
  name     = "${var.project_id}-live-config-deploy-notification-rule"
  resource = aws_codebuild_project.di_deploy_live_configuration.arn
