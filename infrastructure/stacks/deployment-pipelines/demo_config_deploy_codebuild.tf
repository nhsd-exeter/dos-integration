resource "aws_codebuild_webhook" "demo_configuration-deployment_webhook" {
  ...
  filter_group {
    ...
    filter {
      type    = "HEAD_REF"
      pattern = "^refs/tags/.*-demo-config-deploy"
    }
  }
}
resource "aws_codebuild_project" "di_deploy_demo_configuration" {
  name           = "${var.project_id}-demo-config-deploy-stage"
  description    = "Deploy configuration to the demo environment"
  build_timeout  = "30"
  queued_timeout = "30"
  ...
  logs_config {
    cloudwatch_logs {
      group_name  = "/aws/codebuild/${var.project_id}-demo-config-deploy-stage"
      stream_name = ""
    }
  }
  ...
  source {
    ...
    buildspec       = data.template_file.deploy_configuration_buildspec.rendered
  }
  depends_on = [aws_codebuild_source_credential.github_authenication]
}

resource "aws_codestarnotifications_notification_rule" "demo_config_deploy_notification_rule" {
  detail_type    = "BASIC"
  ...
  name     = "${var.project_id}-demo-config-deploy-notification-rule"
  resource = aws_codebuild_project.di_deploy_demo_configuration.arn
